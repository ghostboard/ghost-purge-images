...
function getCredentials(url, callback) {
  request.get(url, (err, http, body) => {
    ...
    if (!proceed) {
      return callback(`Invalid response fetching URL err=${err} body=${body}`);
    }
    ...
    const coverKey = '<meta property="og:image" content="';
    const coverStart = body.indexOf(coverKey) + coverKey.length;
    const coverEnd = body.indexOf('"', coverStart + 1);
    const publicationCover = body.substring(coverStart, coverEnd);

    const logoKey = '"logo": "';
    const logoStart = body.indexOf(logoKey) + logoKey.length;
    const logoEnd = body.indexOf('"', logoStart + 1);
    const publicationLogo = body.substring(logoStart, logoEnd);
...
module.exports = {
  fetch(cb) {
    let config = null;
    try {
      config = require(`${process.cwd()}/config.production.json`);
    } catch (err) {
    ...
    async.waterfall([
      ...
      function(credentials, callback) {
        ...
        async.parallel({
          ...
          const output = {
            list: out.posts.concat(out.pages),
            publicationLogo: credentials.publicationLogo,
            publicationCover: credentials.publicationCover
          };
          callback(null, output);
        });
      }

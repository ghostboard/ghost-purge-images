...
const isInAudioCard = require('./isInAudioCard');
const isInVideoCard = require('./isInVideoCard');
...
function isBeingUsed(upload, posts) {
  ...
  for (const post of posts) {
    ...
    const inAudioCard = isInAudioCard(upload, post.mobiledoc);
    const inVideoCard = isInVideoCard(upload, post.mobiledoc);
    const isUsed = inBody || inFeat || inOG || inTw || inAudioCard || inVideoCard;
    if (isUsed) {
      return true;
    }
  }

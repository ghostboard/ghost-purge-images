...
module.exports = {
  ...
  display(result, callback) {
    ...
    async.eachSeries(result.list, (item, eachCb) => {
      ...
      console.log('');
      console.log('❔ Want to delete this files? Run `ghost-purge-images purge --content-key=YOUR_CONTENT_KEY --admin-key=YOUR_ADMIN_KEY`');
      console.log('🎁 Open source tool by https://ghostboard.io');
      callback();
    });
  },
